...
import (
	...
	"github.com/theotw/k8srelay/pkg/testing"
...
func getLocationIDFromEnv() string {
	return utils.GetEnvWithDefaults(locationIDEnvVar, defaultLocationID)
}

func getRelayPort() string {
	port := utils.GetEnvWithDefaults(proxyPortEnvVar, defaultRelayPort)
...
func (s *server) RunRelayServer(test bool) error {
	...
	tmp := utils.GetEnvWithDefaults("CERT_DIR", "out/")
